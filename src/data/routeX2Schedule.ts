// src/data/routeX2Schedule.ts
// Route X2 schedule parsing from CSV timecodes.
...
const ROUTE_X2_SCHEDULE_URL = '/data/timecodes_bus_X2.csv'
...
export function useRouteX2Schedule() {
  const { data: stopsData } = useStopsData()
  const csvQuery = useQuery({
    queryKey: ['route-X2-schedule'],
    queryFn: async () => {
      const response = await fetch(ROUTE_X2_SCHEDULE_URL)
      if (!response.ok) {
        throw new Error(`Failed to load route X2 schedule: ${response.status}`)
      }
